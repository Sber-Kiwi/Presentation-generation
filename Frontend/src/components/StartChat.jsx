...
export default function StartChat({ onSubmit, disabled, error }) {
  ...
  const fileInputRef = useRef(null);
    ...
    const MAX_CHARS = 500;
    const currentChars = prompt.length;
    const isOverLimit = currentChars > MAX_CHARS;
    const canSend = !disabled && prompt.trim().length > 0 && !!file && !isOverLimit;

    // ... handleAttachClick, handleFileChange, handleRemoveFile remain same ...
